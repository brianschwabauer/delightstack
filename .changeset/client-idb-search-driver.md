...
**Synced entities stop exposing vector fields.** Sync ships the server's sparse document *minus* its `vector[...]` fields. Vector search is server-only, so the client never needed embeddings — but this is observable: an app reading `entity.embedding` off a *synced* (sparse) document will now find it absent. Fetching the entity directly is unaffected.
