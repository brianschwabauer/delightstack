...
<div style="display: flex; gap: 0.5rem; margin-top: 1rem;">
	<Button outline onclick={() => current = Math.max(0, current - 1)} disabled={current === 0}>Back</Button>
	<Button onclick={() => current = Math.min(4, current + 1)} disabled={current === 4}>Next</Button>
</div>
