...
### Pill, Dense & Comfortable

`dense` and `comfortable` adjust the height and padding using the shared control
sizing tokens, so a Button lines up with `dense`/`comfortable` Input and Select
fields in the same form row.

<ComponentPreview code={`<Button dense>Dense</Button>
...
<Button pill dense>Pill Dense</Button>`}>
  ...
  <Button client:load comfortable>Comfortable</Button>
