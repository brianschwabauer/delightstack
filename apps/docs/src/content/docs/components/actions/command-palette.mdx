...
import CommandPaletteComfortableDemo from '../../../../components/demos/CommandPaletteComfortableDemo.svelte';
...
### Roomy / Comfortable Mode

Use `comfortable` for more generous spacing and larger tap targets — handy on
touch interfaces or when the palette is the primary surface.

<ComponentPreview code={`<CommandPalette bind:open={paletteOpen} {commands} comfortable />`}>
  <CommandPaletteComfortableDemo client:load />
</ComponentPreview>
...
<ComponentPreview code={`<CommandPalette bind:open={paletteOpen} {commands} group_by="none" />`}>
