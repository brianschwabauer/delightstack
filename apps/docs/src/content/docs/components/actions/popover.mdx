...
${'  '}${'  '}let refElement = $state(undefined);
</script>

<Button bind:buttonElement={refElement}>Toggle Popover</Button>
<Popover {refElement} openOnClick>
${'  '}${'  '}<h2>Popover Title</h2>
...
<ComponentPreview
	code={`<span bind:this={refElement}>Hover over me for info</span>
<Popover {refElement} openOnHover>
