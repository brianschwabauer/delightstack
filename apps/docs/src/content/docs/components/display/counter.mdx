...
import CounterSkeletonDemo from '../../../../components/demos/CounterSkeletonDemo.svelte';
...
`value` is optional — it may arrive late (e.g. while stats load). With `skeleton`
set, a number-shaped shimmer pill shows until `value` is available, and the
count-up starts the moment it arrives.

<ComponentPreview code={`<Counter skeleton value={loading ? undefined : 12845} />`}>
	<CounterSkeletonDemo client:load />
</ComponentPreview>
