...
| `focused` | `string \| null` | `null` | The node the keyboard cursor is on (bindable); `null` when the tree is not focused |
...
A collapsed branch is not rendered. The tree puts a branch's children in the
document when it opens and takes them out again once the closing animation has
finished, so the number of elements on the page follows what is open rather than
how much data was passed in — a 10,000-node tree with everything collapsed is
twenty rows of DOM, not ten thousand.

That is invisible from the outside: `expanded`, `selected` and `focused` still
name any node in `data`, keyboard navigation still walks the whole visible order,
and `filter` still searches every node whether or not its branch is open. What it
does mean is that `document.querySelector` only finds rows inside open branches —
code reaching into the tree's DOM should read the node data instead.
...
| Shift + Arrow Up/Down | Extend the selection one row, taking the cursor with it (`multi_select`) |
| Mod + A | Select every visible node (`multi_select`) |
| Escape | Clear the selection — claimed only when there is one to clear (`multi_select`) |
| Printable characters | Type-ahead — jumps to the next node whose label starts with what you typed. The buffer resets after 700ms; repeating one character cycles through the nodes starting with it |

With `multi_select`, clicking follows the same rules as the pointer half: `mod`-click toggles one
node, `shift`-click takes the range from the anchor, and `mod+shift`-click keeps the rest. The
anchor stays where the last plain or `mod`-click put it, so a second, closer `shift`-click narrows
the range rather than ratcheting it outward. The rules live in the exported selection model
(`applySelection`, `extendSelection`, `selectionGestureOf`), which `List` and `Table` can adopt
through the same functions.

Keystrokes that originate inside an `<input>`, `<textarea>`, `<select>` or a
`contenteditable` element — an inline rename rendered by `node_content`, say —
are left alone, so the control keeps its own arrows, Home/End and Enter.
