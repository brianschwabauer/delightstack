...
import FormValidationDemo from '../../../../components/demos/FormValidationDemo.svelte';
import FormValibotDemo from '../../../../components/demos/FormValibotDemo.svelte';
import FormPromiseDemo from '../../../../components/demos/FormPromiseDemo.svelte';
import FormTimingDemo from '../../../../components/demos/FormTimingDemo.svelte';
import FormResetDemo from '../../../../components/demos/FormResetDemo.svelte';
...
The `schema` prop accepts any [Standard Schema](https://github.com/standard-schema/standard-schema) compatible validator. Zod, Valibot, and ArkType all work. Fields with a `name` register themselves with the form and show their error once touched — no `bind:value` per field.
...
${'  '}</div>
  ...
  <FormValidationDemo client:load />
</ComponentPreview>
...
<ComponentPreview code={`<!-- Validate on blur (default) -->
...
<ComponentPreview code={`<Form bind:data={formData} {schema} reset_on_submit>
${'  '}<Input name="name" label="Name" />
${'  '}<Input name="email" label="Email" type="email" />
${'  '}<Button type="reset" ghost>Reset</Button>
${'  '}<Button type="submit">Submit</Button>
</Form>`}>
  <FormResetDemo client:load />
</ComponentPreview>
...
|------|------|---------|-------------|
| `data` | `object` | `{}` | Form data object, bindable. Ignored when `entity` is set |
| `entity` | `FormEntity` | — | An entity to bind the form to (e.g. `db.entity(...)`): values, dirty/submitting state, and `save()` on submit all derive from it |
...
<Aside type="tip">
Database tables from `@delightstack/database` expose a ready-made Standard Schema at `table.form.schema`, plus spreadable per-field props (including `parse`) at `table.form.field`. See the [Working with Forms guide](/guides/forms/) for the full pattern.
</Aside>
