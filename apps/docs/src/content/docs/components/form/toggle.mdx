---
title: Toggle
description: An on/off switch component for binary choices with slide animation, bounce physics, optional thumb icon, and an optional three-state indeterminate mode.
---
...
import ToggleIndeterminateDemo from '../../../../components/demos/ToggleIndeterminateDemo.svelte';
...
Set `indeterminate` to add a third, in-between state. `checked` is then
...
<Toggle indeterminate bind:checked={moderation} label="Comment moderation" />`}>
	<ToggleIndeterminateDemo client:load />
</ComponentPreview>
...
|------|------|---------|-------------|
| `checked` | `boolean` (or `boolean \| null` with `indeterminate`) | `false` | Toggle state, bindable. `null` is the in-between state, only available in indeterminate mode |
