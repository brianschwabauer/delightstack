...
| `caption_display`              | `'none' \| 'always'`                     | `'none'`    | Whether a video slide shows its item's `caption`. The player draws it as part of its own chrome — above the control bar, under the same scrim — so it never covers the controls. |
