...
Every mode (except `lightbox`, which has no thumbnails) opens a full-screen modal carousel on click, with pinch-to-zoom, swipe-to-dismiss, keyboard navigation, and an optional pages view for PDFs.
...
| `num_pages`               | `number` _(bindable)_                             | `1`                          | Number of pages in the current item. Set by the `pdf` snippet.                                        |
...
| `type`             | `'image' \| 'video' \| 'pdf' \| 'embed'` | Defaults to `'image'`.                                                                                                                  |
| `src`              | `string`                                 | URL, or a srcset string with width descriptors (`"sm.jpg 400w, lg.jpg 1600w"`).                                                         |
...
| `panorama`         | `boolean`                                | Render this image with the `panorama` snippet.                                                                                          |
