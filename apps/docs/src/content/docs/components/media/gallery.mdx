...
Click any thumbnail below — the modal opens and that video starts playing. The items also carry a `caption`, so the demo doubles as [captions on video slides](#captions-on-video-slides).
...
<Gallery {items} display="grid" autoplay_video meta_display_fullscreen="always" />`}>
	<GalleryVideoAutoplayDemo client:visible />
</ComponentPreview>

## Captions on video slides

`meta_display_fullscreen="always"` pins each item's caption to the bottom of the lightbox — but a video slide already owns that space with its own control bar. So on video slides the caption isn't an overlay at all: Gallery hands it to the player, which draws it as part of its own chrome, directly above the control bar and under the same scrim. One gradient covers caption and controls together — no seam where two scrims meet — and the caption appears and fades with the controls, so it's gone once playback runs and the pointer goes idle. Move the pointer, or pause, and both come back. Image and PDF slides keep the bottom-pinned caption unchanged.

The demo above shows it: open a clip, let it play until the chrome fades, then move the pointer to bring the caption and the controls back together.
