...
import ImageBlurUpDemo from '../../../../components/demos/ImageBlurUpDemo.svelte';
...
Provide a tiny placeholder for a smooth blur-up transition as the full image loads. You can supply either a small `placeholder` image URL or a base64 [ThumbHash](https://evanw.github.io/thumbhash/) via `thumbhash` (decoded internally — no extra network request). `thumbhash` takes precedence when both are set.

<ComponentPreview code={`<!-- Tiny placeholder image URL -->
<Image
...
| `thumbhash` | `string` | `undefined` | Base64 [ThumbHash](https://evanw.github.io/thumbhash/) decoded internally into a tiny blurred placeholder (takes precedence over `placeholder`, no network request) |
