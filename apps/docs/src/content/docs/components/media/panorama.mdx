...
import PanoramaSkeletonDemo from '../../../../components/demos/PanoramaSkeletonDemo.svelte';
...
<ComponentPreview code={`<Panorama src="/panoramas/cityscape.jpg" auto_rotate auto_rotate_speed={1.5} />`}>
...
The skeleton is loading-state-driven: it shows while the equirectangular texture
is still downloading and dismisses itself the moment the panorama is ready.

<ComponentPreview code={`<Panorama skeleton src={panorama_url} />`}>
	<PanoramaSkeletonDemo client:visible />
</ComponentPreview>
