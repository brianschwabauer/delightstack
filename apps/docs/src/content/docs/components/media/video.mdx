...
Use `skeleton` while the source is still being resolved (e.g. fetched async).
...
| `title` | `string` | `undefined` | Short title/caption drawn as part of the chrome — above the control bar, under the same scrim, fading with the controls. Distinct from `captions`, which are WebVTT subtitle tracks |
