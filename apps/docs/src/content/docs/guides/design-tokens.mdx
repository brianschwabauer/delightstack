...
To re-theme, override the brand seed (or any individual semantic token) in your own CSS:

```css
:root {
	--color-primary: #2563eb; /* re-derives action/accent + the whole ramp */
...
`--color-error` and `--color-success` each carry a full state set (`-active`, `-disabled`, `-text`, `-text-active`, `-text-disabled`). `--color-warning` and `--color-info` are single colors. Each of the four also has a soft tinted background for callouts and alerts:
...
:root {
	...
	--color-info: light-dark(#2f6fe0, #5b8def);
...
```css
:root {
	--control-font-0: 0.8125rem; /* 13px */
	--control-font-1: 0.9375rem; /* 15px — default */
	--control-font-2: 1.0625rem; /* 17px */
	--control-font-3: 1.1875rem; /* 19px */

	--control-height-ratio: 3.2; /* default     → 48px @ size 1 */
	--control-height-ratio-dense: 2.4; /* dense       → 36px @ size 1 */
	--control-height-ratio-comfortable: 4; /* comfortable → 60px @ size 1 */
