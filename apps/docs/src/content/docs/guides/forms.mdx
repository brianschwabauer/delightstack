...
## Schema-Driven Forms (Database Tables)

If you define your data with `Database.table()` from `@delightstack/database`, you do not need to write a validation schema or input props by hand — every table exposes ready-made form wiring:

- **`table.form.field.<name>`** — props to spread onto a field component: `name`, `type`, `label`, `placeholder`, `description`, `required`, `readonly`, length/value constraints, enum `options`, array `multiple`, and a `parse` validator.
- **`table.form.schema`** — a Standard Schema validator covering all of the table's editable fields, ready for the `Form` component's `schema` prop.
...
<script>
  ...
  async function handleSubmit({ data, is_valid }) {
    // Only called when the table schema validates successfully
  }
</script>

<Form schema={personTable.form.schema} onsubmit={handleSubmit}>
  <Input {...field.name} bind:value={name} />
  <Input {...field.email} bind:value={email} />
  <Select {...field.relationship} bind:value={relationship} clearable />
...
`field.email` spreads as `{ name: 'email', type: 'email', label: 'Email', parse }` — the input type, label, and validation all come from the one schema definition.
...
| `s.boolean()`                   | `type: 'boolean'` — spread onto `Checkbox` or `Toggle` (not `Input`) and use `bind:checked`        |
