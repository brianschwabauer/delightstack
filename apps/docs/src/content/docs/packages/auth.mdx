...
Users can belong to many orgs. Each org has exactly one **owner** (the user in the org row's
...
On transfer, the new owner is granted the admin permission (and org membership if needed).
The previous owner keeps their existing permissions — demote or remove them afterwards if
that's what your app wants.
