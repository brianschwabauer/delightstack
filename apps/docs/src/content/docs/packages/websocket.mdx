...
Declare it in `wrangler.toml` (see [Architecture](/getting-started/architecture/)).
...
<Aside type="tip">
Combine with [Database](/packages/database/): pass `ws.databaseHooks()` to your `DatabaseClient` so
