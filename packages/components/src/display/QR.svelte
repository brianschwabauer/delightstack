<script module lang="ts">
	...
	function interleaveBlocks(codewords: Uint8Array, version: number, level: ECLevel): number[] {
		const row = EC_TABLE[level][version - 1];
	...
	function placeDataBits(modules: boolean[][], reserved: boolean[][], data_bits: number[]) {
		const size = modules.length;
		let bit_idx = 0;
	...
	function applyMask(modules: boolean[][], reserved: boolean[][], mask_idx: number): boolean[][] {
		const size = modules.length;
	...
	function writeFormatBits(modules: boolean[][], version: number, level: ECLevel, mask_idx: number) {
		const size = modules.length;
	...
	<div
		...
		{#if downloadable}
			<button
				...
				type="button">
				{#if is_downloading}
					<svg class="download-icon spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
