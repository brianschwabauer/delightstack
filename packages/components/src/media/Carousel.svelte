...
<script lang="ts">
	import { decodeMedia, getMediaPreviews, resize } from '@packages/lib';
	...
	import { loadedMediaResolutions, addLoadedMediaResolution } from './Image.svelte';
	import { ApiMetadata, Media } from '@packages/api';
	import type { MediaEntity } from '../state';
	import { page as pageState } from '$app/state';
	import Video from './Video.svelte';

	/**
	 * TODO
	 * - [ ] add better iframe support
	 */

	interface $$Events {
		/**
		 * Emits when this component is requesting the carousel be closed by the parent
		 * Call event.preventDefault() to cancel the carousel closing
		 */
		close: CustomEvent<void>;

		/** Emits when the user interacts with the carousel */
		interaction: CustomEvent<void>;
	}
	...
	let {
		...
		items = [] as Partial<Media & ApiMetadata>[] | MediaEntity[] | string[],
	...
	interface DecodedMediaItem {
		...
		/** The key to use for the each block (this is necessary a media item with the same id can be in the carousel multiple times) */
		key: string;
		...
		pages: Array<{
			/** The x position of the image (relative to the container). Extracted from the 'transform' matrix */
			x: number;

			/** The y position of the image (relative to the container). Extracted from the 'transform' matrix */
			y: number;

			/** The z position of the image (relative to the container). Extracted from the 'transform' matrix */
			z: number;

			/** The current scale of the image */
			scale: number;

			/** The resolution of image to load in pixels */
			resolutionW: number;

			/** The resolution of image to load in pixels */
			resolutionH: number;

			/** The transform matrix of the item (used to calculate the transform string) */
			matrix: DOMMatrix;

			/** The number of pixels the page is panning */
			panX?: number;
			...
			/**
			 * The number of pixels the element is from the left of the container.
			 * This enables "flex" type layouts where the image element is not the full height of the container.
			 */
			offsetX: number;

			/**
			 * The number of pixels the element is from the top of the container.
			 * This enables "flex" type layouts where the image element is not the full height of the container.
			 */
			offsetY: number;

			/** The width of the unscaled/untransformed element */
			offsetWidth: number;

			/** The height of the unscaled/untransformed element */
			offsetHeight: number;
		}>;
	}

	// The sanitized/formatted list of items to display in the gallery
	let list = $state<Array<DecodedMediaItem>>([]);

	/** The container element (used to find the bounds of carousel) */
	let viewport = $state<HTMLElement | undefined>();

	/** The gallery slider items container element (contains gallery items as children) */
	...
	/** The transform the pointers create based on their relationship to eachother */
	let transform: Transform | undefined;

	/** Whether the current gesture has been emitted (as 'interaction') */
	...
	/** The event timestamp when the user last tapped the gallery */
	let lastTapEvent = 0;
	...
	const baseUrlProtocol = $derived(pageState.url.protocol);
	const baseUrlHost = $derived(pageState.url.host);
	...
	async function initItems(
		items: Partial<Media & ApiMetadata>[] | MediaEntity[] | string[],
	) {
		numPages = 1;
		const newList: DecodedMediaItem[] = [];
		for (const item of items) {
			const media = typeof item === 'string' ? decodeMedia(item) : item;
			if (!media) continue;
			let id = 'id' in media ? media.id : '_id' in media ? media._id : '';
			if (!id) continue;
			const prevItem = list.find((v) => v.id === id);
			const normalDistance = Math.abs(newList.length - index);
			const distance = Math.min(normalDistance, items.length - normalDistance);
			const shouldLoad = distance <= 0;
			const previews = getMediaPreviews(media);
			const type = (media as any).type === 'img' ? 'image' : media.type;
			const initialResolution = Math.max(0, ...(loadedMediaResolutions.get(id) || []));
			newList.push({
				id,
				type,
				url:
					type === 'image' && !media.panoramaEnabled
						? previews
								.map(
									(p) =>
										`${transformURL(p.url)}${p.resolution ? ` ${p.resolution}w` : ''}`,
								)
								.join(', ')
						: media.url,
				...
				height: prevItem?.height || 0,
				ratio: prevItem?.ratio || media.ratio || 1,
				panoramaEnabled: media.panoramaEnabled ?? false,
				tinyUrl: transformURL(previews[0]?.url || ''),
				...
				transform: undefined,
				page: 0,
				initialResolution: Math.max(0, ...(loadedMediaResolutions.get(id) || [])),
				pages: [
					{
						scale: 1,
						x: 0,
						...
						matrix: browser ? createMatrix() : undefined,
					},
				],
			} as DecodedMediaItem);
		}
	...
	export async function goToSlide(
		...
		list.forEach((item, i) => {
			...
			if (!el) return;
			(el as HTMLElement).getAnimations().forEach((animation) => {
				try {
	...
	/** Handles an "down" action - like a swipe down, arrow key down, or a button press that has an down arrow */
	...
	/** Handles an "left" action - like a swipe left, arrow key left, or a button press that has an left arrow */
	...
	/** Handles an "right" action - like a swipe right, arrow key right, or a button press that has an right arrow */
	...
	function close() {
		...
		if (onclose) {
			...
			success = dispatch('close', undefined, { cancelable: true });
		}
		if (!success) {
			animateItem(index, { easing: 'back-out' });
			dismissing = 0;
		} else {
			// Reset the dismissing value after a short delay for the animation to finish
			setTimeout(() => {
				dismissing = 0;
			}, 100);
		}
	...
	/** Returns the target element of the item at the given index. This is the element that will be scaled/transformed */
	...
	/** Animates the item at the given index to the given matrix/transform. Handles updating the item's meta after */
	...
	/** Zooms into the current gallery item by the given scale, centered on targetX/targetY */
	...
	/** Zooms out to the given scale on the current gallery item */
	...
	/** Animates the current gallery item by the given x, y (only if zoomed in already) */
	...
	/** Animates the gallery container's x dimension */
	async function animateContainer(type: 'reset' | 'slide' = 'reset', duration?: number) {
		...
		if (!navigator.vendor.match(/apple/i)) {
			...
			await tick();
			// HACK for the worst f***ing browser on the planet - Safari
			// We can't use the animation api because the Safari keeps removing hardware accerleration
			// Many ours of debugging later, we're left with this hacky solution
			let start = Date.now();
	...
	/** Animates the gallery page container's y dimension */
	...
	function onInteractionEnd(e: PointerEvent) {
		...
		// Handle dismissing the gallery (or snapping back to the initial state)
		...
		// Close the gallery if the user taps on the background
		if (isTap && e.target) {
	...
	function onPointerEvent(pointer: Pointer) {
		...
		if (gesture === 'pan-y-dismiss') {
			...
			if (scale > 1.01) {
				...
				// Animate the current gallery item back to the center
				if (!matrix.isIdentity) {
					matrix = createMatrix();
		...
		// Update the closing progress if necessary
		...
		// Mark the container has being swiped if necessary
	...
	/** Handles all key events when the gallery is active in modal mode */
	...
	function onImageLoadEvent(i: number, e: Event) {
		...
		item.loaded = true;
		if (item.id && item.width) addLoadedMediaResolution(item.id, item.width);

		// When the current image is loaded, we can now allow it to fetch the full-res image
		// Previously, we limited it to only 2048px in width/height
		// So that the initial load is faster
	...
	/** Adds/removes the 'transitioning' class to the gallery container */
	...
	/** Initializes the event listeners when the gallery is shown in modal mode */
	$effect(() => {
	...
	/** Destroyes the event listeners when the gallery is not being shown */
			...
			{#if distance === 0 || (!opening && distance <= 5) || (item.shouldLoad && distance <= 20)}
				<li
					...
					style:grid-column-start={((list.length + i - index + offset) % list.length) +
						1}>
					{#if item.shouldLoad}
						{#if !item.loaded && item.tinyUrl && !opening}
							<img
								src={item.tinyUrl}
						...
						{#if item.url}
							{#if item.type === 'image'}
								{#if item.type === 'image' && !item.panoramaEnabled}
									...
									<img
										...
										sizes={resolution ? `${resolution}px` : '100vw'}
							...
							{:else if item.type === 'pdf'}
								<Pdf
									...
									pageBounds={item.pages.map((_, i) => ({
										width: item.pages[i].resolutionW,
										height: item.pages[i].resolutionH,
									}))} />
							{:else if item.type === 'video'}
								...
								<Video
									{preview}
									src={hls}
									playing={i !== index ? false : item.shouldPlay ? true : undefined}
									onload={() => item.loaded || (list[index].loaded = true)}>
								</Video>
	...
	.item {
		...
		> img {
			...
			&.explicit-size {
				...
				height: 100%;
				max-height: calc(
					(100cqw * (1 / var(--ratio, 1))) - var(--carousel-padding-top, 0px) - var(
