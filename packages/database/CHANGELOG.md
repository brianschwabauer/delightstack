...
- 51e8b8e: `.serverOnly()` — index a field in the Durable Object without shipping it to every client.
  ...
  The value is held back from the _wire_, not from the database — `db.get()` and any `sparse: false` read still return it. One list drives one strip, sharing the path vector fields already took out of the sync payload for the same reason: indexed where it lives, too heavy to copy everywhere.
...
- e77bd9d: Durable offline mutation queue, and `{ op_id }` idempotent writes.
  ...
  **Idempotency is enforced on the server, because that is the only place it can be.** `create`, `update`, `delete` and `transaction` take an `{ op_id }` write option. The first write carrying an id is applied and recorded in a new internal `_op_log`; every later write carrying it returns the original result without touching a row. The log row commits in the same transaction as the write, so a write that _threw_ records nothing and a genuine retry still applies. Retention is seven days, swept by a registered alarm; `db.appliedOperation(op_id)` reads the record back. `batch()` refuses an `op_id` outright — its callback's return value is arbitrary, and dedupe that hands back the wrong thing is worse than no dedupe.
...
- 1987750: Add a per-table change log with operation-level undo, actor attribution on every write, binary/file column types, and a file deletion queue.
...
- 03ae5c9: Fix two ways the reactive list handle could go permanently stale or empty.
...
- fe9d902: The legacy `search_index`/`search_journal` teardown after a completed migration is now chunked, fixing a CPU-limit wedge on large journals.
...
- b0708c9: `scheduleRebuildAlarm` now arms the Durable Object alarm unconditionally, fixing a wedge where a deferred search rebuild never continued.
...
- a4de607: The chunked search rebuild's row budget is now shared across all pending entity types per invocation, instead of granting each type its own full slice.
