...
async function createWorker(
	...
	await worker.init({
		tables: { note: { orama: table.config.orama as any, primary_key: 'id' } },
		db_name,
		...init,
	});
	return worker;
...
describe('DatabaseWorker client search (IndexedDB driver)', () => {
	...
	it('a config bump reopens the database, rebuilds the index and resyncs', async () => {
		...
		const index = (server as any).getIndex('note');
		...
		expect(meta?.config_version).toBe(index.config_version);
	});

	it('drops the legacy Orama blob store on the first upgrade', async () => {
