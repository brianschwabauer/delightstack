...
**Status: all five phases implemented** (working tree, pending review) as of 2026-08-12. Phase 1 (owned types + snake_case renames), Phase 2 (`search/core/*`, the memory reference engine and store, the differential harness, the golden vectors), Phase 3 (the SQLite server driver, production wiring, the perf pass), Phase 4 (the IndexedDB client driver and worker wiring) and Phase 5 (Orama excised: the `search_engine` flag, the in-memory index, the msgpack snapshot, the journal, the Appendix-A workarounds and the `@orama/orama` + `@msgpack/msgpack` dependencies are all gone; the legacy `search_index`/`search_journal` tables are migrated and dropped on the first wake) are in `packages/database/src/`. Every `[verify-vs-orama]` marker below is resolved against `plans/database/orama-verification-report.md` and the spec is frozen.
...
**Phase 5 — Excision** — *implemented 2026-08-12 (working tree, pending review)*
`@orama/orama` and `@msgpack/msgpack` removed from `dependencies`; the differential harness (§8.1), `orama_reference.ts` and `search/orama-compat.ts` deleted with them (their job ended when the spec froze in Phase 2 — the golden vectors are the permanent guard, and they regenerate from the memory reference). The per-table `search_engine` flag is deleted and the SQLite driver bootstraps unconditionally. Deleted from `db.server.ts`: `getIndex` / `rebuildIndex` / `saveIndex`, the msgpack chunked snapshot, the whole journal (write / replay / compaction / rollback invalidation) and Appendix-A ①–⑧. The legacy `search_index` / `search_journal` tables are migrated (metadata first) and then **dropped**, once, on the first wake after upgrade, after every configured table has migrated and rebuilt. Deferred §6 renames landed: `OramaType<T>` → `IndexFieldType<T>`, `table.config.orama` → `table.config.index_schema` (now the flat schema itself; `sort`/`components` deleted). README and docs updated; changeset `.changeset/orama-excision.md` (major).

Two things the excision found and fixed: `search_state.first_updated_at` stayed pinned at `0` once a state row was created with `0` (`MIN(0, x)`), which a descending-backfilling client reads as "you have reached the beginning" after its first page; and a first-ever bootstrap bumped `config_version` to 2 for a corpus no client had ever seen.

Test disposition: `db.server.journal.test.ts` and `db.server.coldstart.test.ts` deleted (they guarded deleted machinery); `db.server.test.ts`, `db.server.sync.test.ts`, `db.server.batch.test.ts` and the three `database.worker*.test.ts` suites moved off their hand-written SQL fakes onto the real-SQLite harness; `db.server.native.test.ts` gained the full legacy-migration wake (seeded `search_index` + `search_journal` → metadata migrated, rows rebuilt, legacy tables dropped, `config_version` bumped past the legacy one, idempotent on the next wake) and the §8.3 zero-search-work-on-warm-boot assertion.
